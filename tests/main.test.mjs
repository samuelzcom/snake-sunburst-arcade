...
const withFixedRandom = async (callback) => {
...
test("main bootstraps the game loop and wires keyboard controls", async () => {
  await withFixedRandom(async () => {
    ...
    await importMainModule(`bootstrap=${moduleImportCounter += 1}`);
...
test("main only restarts from Enter after the run has ended", async () => {
  await withFixedRandom(async () => {
    ...
    await importMainModule(`restart-guard=${moduleImportCounter += 1}`);
...
test("main keyboard controls resume paused play and do not change a finished run", async () => {
  await withFixedRandom(async () => {
    ...
    await importMainModule(`pause-resume=${moduleImportCounter += 1}`);
...
test("main throws when the board canvas is missing", async () => {
  ...
  await assert.rejects(
    importMainModule(`missing-canvas=${moduleImportCounter += 1}`),
